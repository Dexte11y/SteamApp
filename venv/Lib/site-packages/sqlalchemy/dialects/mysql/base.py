...
To set isolation level using :func:`_sa.create_engine`::
    ...
    engine = create_engine(
                    "mysql+mysqldb://scott:tiger@localhost/tests",
...
string parameter ``binary_prefix=true`` to the URL to repair this warning::
    ...
    # mysqlclient
    engine = create_engine(
        "mysql+mysqldb://scott:tiger@localhost/tests?charset=utf8mb4&binary_prefix=true")

    # PyMySQL
    engine = create_engine(
        "mysql+pymysql://scott:tiger@localhost/tests?charset=utf8mb4&binary_prefix=true")
...
the ``first_connect`` and ``connect`` events::
    ...
    eng = create_engine("mysql+mysqldb://scott:tiger@localhost/tests", echo='debug')
...
``nullable=False``.   The following example illustrates::
    ...
    e = create_engine("mysql+mysqldb://scott:tiger@localhost/tests", echo=True)
    m.create_all(e)
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def _render_json_extract_from_binary(self, binary, operator, **kw):
        ...
        if binary.type._type_affinity is sqltypes.Integer:
            ...
            # (zzzeek): I'm not really sure.  let's take a look at a tests case
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _parse_server_version(self, val):
        ...
        # setting it here to help w the tests suite
