...
created so that multiple tests frameworks can be supported at once
...
def setup_options(make_option):
    ...
    make_option(
        "--disable-asyncio",
        action="store_true",
        help="disable tests / fixtures / provisoning running in asyncio",
    )
    make_option(
        "--backend-only",
        action="callback",
    ...
    make_option(
        "--mypy-extra-tests-path",
        type=str,
        action="append",
        default=[],
        dest="mypy_extra_test_paths",
        help="Additional tests directories to add to the mypy tests. "
...
def read_config(root_path):
    ...
    file_config.read(
        [str(root_path / "setup.cfg"), str(root_path / "tests.cfg")]
    )
