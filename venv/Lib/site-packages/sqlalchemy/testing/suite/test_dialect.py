...
class ArgSignatureTest(fixtures.TestBase):
    """tests that all visit_XYZ() in :class:`_sql.Compiler` subclasses have
    ``**kw``, for #8988.

    This tests uses runtime code inspection.   Does not need to be a
    ``__backend__`` tests as it only needs to run once provided all target
    ...
    imported by the time this tests runs.
...
class IsolationLevelTest(fixtures.TestBase):
    ...
    def test_invalid_level_execution_option(self, connection_no_trans):
        """tests for the new get_isolation_level_values() method"""
...
class ReturningGuardsTest(fixtures.TablesTest):
    """tests that the various 'returning' flags are set appropriately"""
