...
def generate_db_urls(db_urls, extra_drivers):
    """Generate a set of URLs to tests given configured URLs plus additional
    driver names.
...
def create_db(cfg, eng, ident):
    ...
    Used when a tests run will employ multiple processes, e.g., when run
...
def update_db_opts(db_url, db_opts, options):
    """Set database options (db_opts) for a tests database that we created."""
...
def follower_url_from_main(url, ident):
    """Create a connection URL for a dynamically-created tests database.

    :param url: the connection URL specified when the tests run was invoked
...
def run_reap_dbs(url, ident):
    """Remove databases that were created during the tests process, after the
    process has ended.
...
def upsert(
    ...
    insert().upsert(), it's important that we continue to tests the
    ...
    we need to tests like insertmanyvalues, etc.
