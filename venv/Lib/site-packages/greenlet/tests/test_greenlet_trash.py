...
This tests is fragile and relies on details of the CPython
implementation (like most of the rest of this package):
    ...
    - The tests fails by hitting an ``assert()`` in object.c; if the
      build didn't enable assert, then we don't catch this.

    - If the tests fails in that way, the interpreter crashes.
"""
...
class TestTrashCanReEnter(unittest.TestCase):
    ...
    @unittest.skipUnless(
        ...
        "Python 2 tracks this slightly differently, so our tests doesn't catch a problem there. "
    )
    def test_it(self):
    ...
    def check_it(self): # pylint:disable=too-many-statements
        ...
        class Dealloc: # define the class here because we alter class variables each time we run.
            ...
            #: be some.
            #: If the tests is broken or things change in the trashcan implementation,
